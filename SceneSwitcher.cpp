...
static void Scene4_HandleVertexRebuildIfNeeded(BOOL requiresRebuild);
static BOOL SceneSwitcher_HandleScene1Char(int key);
static BOOL SceneSwitcher_HandleScene4Char(int key);
static VkCommandBuffer BeginOneShotCB(void)
{
...
LRESULT CALLBACK WndProc(HWND hwnd, UINT iMsg, WPARAM wParam, LPARAM lParam)
{
        ...
        case WM_CHAR:
        {
            ...
            else if (gActiveScene == ACTIVE_SCENE_SCENE4 ||
                     gActiveScene == ACTIVE_SCENE_SCENE2_SCENE4)
            {
                handled = SceneSwitcher_HandleScene4Char(key);
            }
...
static void Scene4_HandleVertexRebuildIfNeeded(BOOL requiresRebuild)
{
    if (!requiresRebuild || !gCtx_Switcher.bInitialized)
    {
        return;
    }

    VkResult vkResult = WaitForGpuIdle();
    if (vkResult != VK_SUCCESS)
    {
        if (gCtx_Switcher.gpFile)
        {
            fprintf(gCtx_Switcher.gpFile,
                    "Scene4_HandleVertexRebuildIfNeeded() --> WaitForGpuIdle() failed %d\n",
                    vkResult);
        }
        return;
    }
    ...
    if (vkResult != VK_SUCCESS)
    {
        ...
        {
            fprintf(gCtx_Switcher.gpFile,
                    "Scene4_HandleVertexRebuildIfNeeded() --> Scene4_CreateVertexResources() failed %d\n",
                    vkResult);
        }
        return;
    }
    ...
    if (vkResult != VK_SUCCESS && gCtx_Switcher.gpFile)
    {
        fprintf(gCtx_Switcher.gpFile,
                "Scene4_HandleVertexRebuildIfNeeded() --> buildCommandBuffers() failed %d\n",
                vkResult);
    }
}

static void SceneSwitcher_LogScene4KeyAction(int key,
                                             const char* actionLabel,
                                             BOOL applied,
                                             BOOL requestedGeometryRebuild)
{
    if (!gCtx_Switcher.gpFile || !actionLabel)
    {
        return;
    }

    const char* status = applied ? "applied" : "clamped";
    if (applied && requestedGeometryRebuild)
    {
        fprintf(gCtx_Switcher.gpFile,
                "Scene4 key '%c' -> %s (%s, geometry rebuild)\n",
                key,
                actionLabel,
                status);
    }
...
static BOOL SceneSwitcher_HandleScene4Char(int key)
{
    ...
    case 'Z':
        ...
        changed = Scene4_AdjustFireSliceCount(-1, &requiresRebuild);
        SceneSwitcher_LogScene4KeyAction(key,
                                         "decrease fire slice count",
                                         changed,
                                         (changed && requiresRebuild));
        if (changed)
        {
            Scene4_HandleVertexRebuildIfNeeded(requiresRebuild);
            return TRUE;
        }
        break;
    case 'x':
    ...
    case 'c':
    case 'C':
        changed = Scene4_AdjustFireRadius(-1);
        SceneSwitcher_LogScene4KeyAction(key,
                                         "decrease fire radius",
                                         changed,
                                         FALSE);
        if (changed)
        {
    ...
    case 'b':
    case 'B':
        changed = Scene4_AdjustFireHeight(-1);
        SceneSwitcher_LogScene4KeyAction(key,
                                         "decrease fire height",
                                         changed,
                                         FALSE);
        if (changed)
        {
    ...
    case 'g':
    case 'G':
        changed = Scene4_AdjustFireScaleX(-1);
        SceneSwitcher_LogScene4KeyAction(key,
                                         "decrease fire scale X",
                                         changed,
                                         FALSE);
        if (changed)
        {
    ...
    case 'j':
    case 'J':
        changed = Scene4_AdjustFireScaleY(-1);
        SceneSwitcher_LogScene4KeyAction(key,
                                         "decrease fire scale Y",
                                         changed,
                                         FALSE);
        if (changed)
        {
    ...
    case 'u':
    case 'U':
        changed = Scene4_AdjustFireScaleZ(-1);
        SceneSwitcher_LogScene4KeyAction(key,
                                         "decrease fire scale Z",
                                         changed,
                                         FALSE);
        if (changed)
        {
    ...
    case 'I':
      ...
      case 'a':
      case 'A':
          changed = Scene4_AdjustFireOffsetX(-1);
          SceneSwitcher_LogScene4KeyAction(key,
                                           "move fire left (offset X -)",
                                           changed,
                                           FALSE);
          if (changed)
          {
      ...
      case 'w':
      case 'W':
          changed = Scene4_AdjustFireOffsetZ(-1);
          SceneSwitcher_LogScene4KeyAction(key,
                                           "move fire deeper into the scene (offset Z -)",
                                           changed,
                                           FALSE);
          if (changed)
          {
      ...
      case 'S':
          changed = Scene4_AdjustFireOffsetZ(+1);
          SceneSwitcher_LogScene4KeyAction(key,
                                           "move fire toward the camera (offset Z +)",
                                           changed,
                                           FALSE);
          if (changed)
          {
      ...
      case 'q':
      case 'Q':
          changed = Scene4_AdjustFireOffsetY(-1);
          SceneSwitcher_LogScene4KeyAction(key,
                                           "move fire downward (offset Y -)",
                                           changed,
                                           FALSE);
          if (changed)
          {
    ...
    case 'o':
    case 'O':
        changed = Scene4_AdjustFireScrollSpeed(-1);
        SceneSwitcher_LogScene4KeyAction(key,
                                         "decrease fire scroll speed",
                                         changed,
                                         FALSE);
        if (changed)
        {
    ...
    case 'P':
        ...
        break;
    case '1':
        changed = Scene4_AdjustFireTurbulence(-1);
        SceneSwitcher_LogScene4KeyAction(key,
                                         "decrease fire turbulence",
                                         changed,
                                         FALSE);
        if (changed)
        {
    ...
    case '2':
        ...
        break;
    case '3':
        changed = Scene4_AdjustFireTimeSpeed(-1);
        SceneSwitcher_LogScene4KeyAction(key,
                                         "decrease fire time speed",
                                         changed,
                                         FALSE);
        if (changed)
        {
    ...
    case '0':
        requiresRebuild = FALSE;
        changed = Scene4_ResetFireParameters(&requiresRebuild);
        SceneSwitcher_LogScene4KeyAction(key,
                                         "reset fire parameters",
                                         changed,
                                         (changed && requiresRebuild));
        if (changed)
        {
